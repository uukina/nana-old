...
class NanaEmbeds {
  ...
  async getInfoEmbed(id, msg) {
    ...
    if (banned)
      return msg.channel
        .send(
          `:warning: Maaf, Aku tidak dapat menampilkan doujin karena mengandung tag yang di **BANNED** \nCode: ${id}`
        )
    ...
    embed.setFooter(`React dengan 📖 untuk lanjut membaca / ${res.id}`);
  ...
  async getEmoji(id, m, msg) {
    ...
    favorit.on("collect", async f => {
      ...
      if (favorite.includes(res.id)) {
        ...
        return msg.channel
          .send(
            `${nick} kamu telah menambahkan **${res.title.pretty}** ke **Favorite**`
          )
          .then(msg => msg.delete({ timeout: 5000 }));
      }
    });
  ...
  async download(res, type) {
    ...
    const embed = new MessageEmbed()
      ...
      .setTimestamp()
      .setDescription(
        `Untuk memulai download, klik judul doujin diatas.`
      );
    return embed;
  }
}

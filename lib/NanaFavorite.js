...
class NanaFavorite {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let data = await this.client.database.db.run(
          "Masukkan ke favorite (userID, bookID) VALUES ($userID, $bookID);", { $userID: member, $bookID: book }
        );
        resolve(data);
      } catch (err) {
        reject(err);
  ...
  async getUserFavoritID(member) {
    return new Promise(async (resolve, reject) => {
      this.client.database.db.all(
        `Pilih bookID dari favorite WHERE userID = "${member}"`,(err, rows) => {
  ...
  deleteUserFavoritID(member, bookId) {
    return new Promise(async (resolve, reject) => {
      this.client.database.db.all(
        `Hapus dari favorite dimana userID = "${member}" dan bookID = "${bookId}"`, (err, rows) => {
          if (err) return console.log(err);
          resolve(rows);
        }
      );
    ...
    try {
      ...
      if (!favorite[0])
        return msg.channel
          .send(
            `Maaf, ${
              id == msg.author.id ? "kamu" : `**${nick}**`
            } tidak punya ID doujin di favorite.`
          )
      ...
      embed.setAuthor(`${nick} daftar favorite`, member.user.avatarURL);
